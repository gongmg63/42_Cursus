...
/*   Updated: 2024/01/12 13:11:15 by mkong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static ssize_t	read_file(t_read_info *head)
{
	ssize_t	rbytes;
...
static t_read_info	*set_read_info_(int fd, ssize_t *rbytes, t_read_info *head)
{
	if (head == 0)
	{
		head = (t_read_info *)malloc(sizeof(t_read_info));
		if (head == 0)
			return (0);
		head->fd = fd;
		head->buf = 0;
...
static char	*make_str(t_read_info *h, ssize_t *rbytes)
{
	char	*str;
...
static char	*free_lst(t_read_info **head, char *str, ssize_t rbytes)
{
	if (*head == 0)
		return (0);
	free((*head)->buf);
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0)
		return (0);
	head = set_read_info_(fd, &rbytes, head);
	if (head == 0)
		return (0);
